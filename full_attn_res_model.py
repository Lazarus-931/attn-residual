...
class Expert(nn.Module):
  ...
  def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
    ...
    """
    w1 = Linear(in_features=self.dim, out_features=self.inter_dim)
    w2 = Linear(in_features=self.inter_dim, out_features=self.dim)
    w3 = Linear(in_features=self.dim, out_features=self.inter_dim)
...
class FullAttnResiduals(nn.Module):
    """
    Full Attention Residual (Moonshot, arXiv 2603.15031)

    Replaces the standard residual h_l = h_{l-1} + f_l(h_{l-1}) with:
      h_l = Σ_{i=0}^{l} a_{i→l} · h_i
    where h_l here includes f_l's output as h_l = f_l(h_{l-1}).

    The attention weights a_{i→l} are computed per-position via:
      a_{i→l} = softmax_i( RMSNorm(h_i) · w_l )
    where w_l is a learned (d_model,) key vector for layer l.

    all_outputs: list of (batch, seq, dim) tensors [h_0, h_1, ..., h_l]
                 where h_0 is the embedding and h_i (i>0) is the output of layer i.
    """
    dim: int

    @nn.compact
    def __call__(self, all_outputs: list[jnp.ndarray]) -> jnp.ndarray:
        w = self.param("w", nn.initializers.normal(stddev=0.01), (self.dim,))

        # V: (n_layers, batch, seq, dim)
        V = jnp.stack(all_outputs, axis=0)

        # Score each layer's contribution per-position
        # RMSNorm each layer's hidden state, dot with key vector
        normed = RMSNorm(self.dim)(V)                      # (n_layers, batch, seq, dim)
        scores = jnp.einsum('lbsd,d->lbs', normed, w)      # (n_layers, batch, seq)

        # Softmax over layers (axis 0) — per-position weighting
        alpha = jax.nn.softmax(scores.astype(jnp.float32), axis=0).astype(V.dtype)

        # Weighted sum over layers
        return jnp.einsum('lbs,lbsd->bsd', alpha, V)
...
class BottomBlock(nn.Module):
  """KDA_Layer followed by MoE_Layer, with AttnRes residual connections."""
  dim: int
  heads: int
  head_dim: int
  kernel_size: int
  ...
  def setup(self):
    ...
    self.moe = MoE_Layer(dim=self.dim, n_routed_experts=self.n_routed_experts,
                         ...
                         score_func=self.score_func)
    self.attn_res = FullAttnResiduals(self.dim)
    self.ffn_res = FullAttnResiduals(self.dim)

  def __call__(self, x: jnp.ndarray, history: list[jnp.ndarray],
               kda_state: Optional[jnp.ndarray] = None):
    # KDA + attention residual
    kda_out, S = self.kda(x, kda_state)
    x = self.attn_res(history + [kda_out])

    # MoE + attention residual
    moe_out = self.moe(x)
    x = self.ffn_res(history + [kda_out, moe_out])
...
class TopBlock(nn.Module):
  ...
  def __call__(self, x: jnp.ndarray, history: list[jnp.ndarray],
               start: int = 0, freqs_cis: Optional[jnp.ndarray] = None,
               mask: Optional[jnp.ndarray] = None):
    # MLA + attention residual
    mla_out = self.mla(x, start, freqs_cis, mask)
    x = self.attn_res(history + [mla_out])

    # MoE + attention residual
    moe_out = self.moe(x)
    x = self.ffn_res(history + [mla_out, moe_out])
  ...
  def __call__(self, tokens: jnp.ndarray, start: int = 0,
    ...
    for layer in self.layers:
      if isinstance(layer, BottomBlock):
        ...
        x = layer(x, history, start, freqs_cis, mask)
      history.append(x)
