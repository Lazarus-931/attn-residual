...
class BlockAttentionResidual(nn.Module):
    ...
    def __call__(
        self,
        block_reps:    jnp.ndarray,
        partial_block: jnp.ndarray,
    ) -> jnp.ndarray:

        W = self.param("W", nn.initializers.zeros, (self.block_size, self.d_model))

        norm = RMSNorm(self.d_model)

        V_inter = jnp.concatenate([block_reps, partial_block[None]], axis=0)
        K_inter = norm(V_inter)
        logits = jnp.einsum('sd,lbsd->lbs', W, K_inter)
        out = jnp.einsum('lbs,lbsd->bsd', jax.nn.softmax(logits.astype(jnp.float32), axis=0), V_inter)
...
class BottomBlock(nn.Module):
    ...
    def setup(self):
        ...
        self.moe = MoE_Layer(dim=self.dim, n_routed_experts=self.n_routed_experts,
                             ...
                             score_func=self.score_func)
        self.residuals = BlockAttentionResidual(self.dim, self.block_size)

    def __call__(self, x: jnp.ndarray, kda_state: Optional[jnp.ndarray] = None):
        x = self.residuals(block_reps=jnp.expand_dims(x, 0), partial_block=x)
...
class Transformer(nn.Module):
  ...
  def setup(self):
    ...
    self.head = nn.Dense(self.args.vocab_size, use_bias=False)
...
class TrainingState:
    """
    Chunked Attention Residual training
    """
    def setup(self):
        pass
